...
6. **`export const demoGarment: Garment = {` から、そのブロックを閉じる `}` まで**を選び、コピーした設定に置き換えて保存します。ファイル上部の `Point` と `Garment` の型定義、および下の `garments` 配列は消さないでください。
...
5. 保存して再読み込みすると4着になります。上の例では自分の服は**2番目**です。サムネイルを押して確認し、「次の服」や手のスワイプでも選べることを確認してください。
