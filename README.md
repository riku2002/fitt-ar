...
画像は自作の図形による800×800の透過PNGです。詳細・再生成方法・アンカー座標は [public/garments/README.md](public/garments/README.md) を参照してください。画像生成サービスや第三者の商品写真は使用していません。
...
4. 骨格表示だけをOFFにしても服が追従すること。服だけをOFFにしても骨格が動くこと。
...
10. 開発者ツールのリクエストブロックで `demo-tshirt-sage.png` を止め、服表示をOFF→ONにするとエラーと再読み込みボタンが出ること。骨格は継続し、ブロック解除後に再読み込みで復帰すること。
