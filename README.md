...
4. 切り替わったら、動かした手を腰付近まで下ろし、少し待ってから次の操作をします。手を胸の高さで往復させるだけでは連続して切り替わりません。
...
判定は既存のPose Landmarkerの手首座標を使用します。新しいモデルは追加していません。初期値は、650ms以内・4フレーム以上・肩幅の0.8倍以上の横移動、上下のぶれは肩幅の0.35倍以内です。1回切り替わった後は900msの待機と手を下ろす動作が必要です。調整値は `src/features/gesture/swipeDetector.ts` にあります。
...
服・骨格それぞれのCanvasの元解像度を動画に合わせ、同じ `object-fit: contain` と共通の親要素での反転を使います。共有座標そのものは反転せず、ジェスチャーの判定時に画面方向へ変換します。手動の服選択・鏡表示の変更・検出の中断では途中のスワイプ履歴を破棄します。
...
12. 同じ高さで手を戻しても連続切り替えが起きず、手を腰まで下ろしてから再び上げると次の操作ができること。
...
CPU・メインスレッドで同期推定するため、低性能端末では遅くなる可能性があります。低照度・手首の遮蔽・速すぎる動き・処理落ちではスワイプを検出できない場合があります。デモ機で操作感を確認し、必要なら判定値を調整してください。
